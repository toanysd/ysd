...
(function (global) {
  ...
  var TMPL_MAIN = [
    ...
    '  <div class="pu-confirm-overlay pu-hidden" id="puSecurityOverlay" style="background:rgba(255,255,255,0.95); align-items:flex-start; padding-top:20px; backdrop-filter:blur(8px);">',
    '    <div class="pu-confirm-box" style="text-align:center; background:transparent; box-shadow:none; border:none; color:#0f172a; padding:0;">',
    '      <div class="pu-confirm-title" id="puSecOverlayTitle" style="margin-bottom:12px; font-size:15px; color:#1f2937; line-height:1.4;">',
    '        <div style="font-weight:bold; font-size:16px; margin-bottom:10px; color:#0f172a;"><i class="fas fa-camera"></i> <span class="pu-ja">自動認証 (ベータ)</span><span class="pu-vi" style="margin-left:8px; font-weight:normal; font-size:14px;">Xác thực tự động</span></div>',
    '        <p style="font-size: 13px; color: #64748b; margin-bottom: 15px; line-height: 1.5; font-weight: bold;"><span class="plm-ja">システムはスキャンを続行するためカメラを使用します。許可（Allow）を押して認証とスキャンを開始してください。初回のみ確認し、次回以降はセッション終了まで自動認証されます。</span><span class="plm-vi" style="margin-top:4px; display:block; font-weight:normal;">Hệ thống cần sử dụng camera để tiếp tục tính năng scan, vui lòng bấm Allow để cho phép xác thực và bắt đầu scan. Chỉ cần xác nhận ở lần đầu tiên, các lần sau tự động xác thực mà không cần thao tác lại cho đến hết phiên.</span></p>',
    ...
    '          <span class="plm-vi" style="margin-top:4px; display:block; font-weight:normal;">Có thể nhập tên nhân viên tự động bằng preview thông qua xác thực (đang phát triển).</span>',
    '        </p>',
    '      </div>',
    '      <div style="width:100%; display:none; flex-direction:column; align-items:center; margin-bottom:12px; overflow:hidden;" id="puSecVideoWrapper">',
    '        <video id="puSecFrontVideo" autoplay playsinline muted style="width:100%; max-width:250px; border-radius:12px; border:2px solid #3b82f6; transform: scaleX(-1); object-fit:cover;"></video>',
    '      </div>',
    '      <div class="pu-confirm-btns" id="puSecOverlayBtns" style="display: none; flex-direction: column; gap: 10px; margin-top:12px;">',
    '        <button class="pu-confirm-btn" id="puSecBtnPreview" type="button" style="width:100%; background:transparent; color:#64748b; border:1px solid #cbd5e1; display:none;"><i class="fas fa-camera"></i> <span style="display:flex; flex-direction:column; align-items:center; line-height:1.2; text-align:center;"><span style="font-size:14px">プレビューを開く</span><span style="font-size:11px; font-weight:normal">Preview</span></span></button>',
    '        <button class="pu-confirm-btn" id="puSecBtnSkipStart" type="button" style="width:100%; background:#0ea5e9; color:#fff; display:none;"><i class="fas fa-forward"></i> <span style="display:flex; flex-direction:column; align-items:center; line-height:1.2; text-align:center;"><span style="font-size:14px">理解しました。スキップして続行</span><span style="font-size:11px; font-weight:normal">Tôi đã hiểu, bỏ qua preview & tiếp tục</span></span></button>',
    '        <div style="display:none; width:100%; gap:8px;" id="puSecPreviewActionWrapper">',
    '          <button class="pu-confirm-btn pu-confirm-primary" id="puSecBtnSkipPreview" type="button" style="flex:1; background:#475569; color:#f1f5f9; border:none;"><i class="fas fa-barcode"></i> <span style="display:flex; flex-direction:column; align-items:center; line-height:1.2; text-align:center;"><span style="font-size:14px">スキップしてスキャン</span><span style="font-size:11px; font-weight:normal">Bỏ qua & tiếp tục</span></span></button>',
    '          <button class="pu-confirm-btn pu-confirm-primary" id="puSecOverlayAccept" type="button" style="flex:1; background:#10b981; color:#fff; border:none;"><i class="fas fa-check-circle"></i> <span style="display:flex; flex-direction:column; align-items:center; line-height:1.2; text-align:center;"><span style="font-size:14px">撮影して認証</span><span style="font-size:11px; font-weight:normal">Chụp & Xác thực FaceID</span></span></button>',
    '          <button class="pu-confirm-btn pu-confirm-cancel" id="puSecOverlayCancel" type="button" style="flex:none; padding:12px 18px; background:#ef4444; color:#fff; border:none;"><i class="fas fa-times"></i></button>',
    '        </div>',
    '      </div>',
    '    </div>',
    '  </div>',
  ...
  PhotoUploadModule.prototype._bindStatic = function () {
    ...
    var secBtnPreview = document.getElementById('puSecBtnPreview');
    var secBtnSkipStart = document.getElementById('puSecBtnSkipStart');
    var secBtnSkipPreview = document.getElementById('puSecBtnSkipPreview');
    var secPreviewActionWrapper = document.getElementById('puSecPreviewActionWrapper');
    var secVideoWrapper = document.getElementById('puSecVideoWrapper');

    if (secBtnPreview) {
       secBtnPreview.addEventListener('click', function() {
           secBtnPreview.style.display = 'none';
           if (secBtnSkipStart) secBtnSkipStart.style.display = 'none';
           if (secVideoWrapper) secVideoWrapper.style.display = 'flex';
           if (secPreviewActionWrapper) secPreviewActionWrapper.style.display = 'flex';
       });
    }
    ...
    if (btnAsTakePhoto) {
      btnAsTakePhoto.addEventListener('click', function() {
        ...
        if(secBtns) {
            if(!isCamAllowed) {
                if(secBtnSkipStart) secBtnSkipStart.style.display = 'block';
                if(secBtnPreview) secBtnPreview.style.display = 'block';
                secBtns.style.display = 'flex';
            } else {
                if(secBtnSkipStart) secBtnSkipStart.style.display = 'none';
                if(secBtnPreview) secBtnPreview.style.display = 'none';
                // BUG 1 FIX: Vẫn hiển thị nút bấm để có User Gesture trên Android
                secBtns.style.display = 'flex';
                if (secPreviewActionWrapper) secPreviewActionWrapper.style.display = 'flex';
                if (secBtnSkipPreview) secBtnSkipPreview.style.display = 'none';
                if (secCancelBtn) secCancelBtn.style.display = 'none';
                if (secAcceptBtn) secAcceptBtn.style.display = 'block';
                if (secAcceptBtn) {
                   secAcceptBtn.innerHTML = '<i class="fas fa-camera"></i> <span style="display:flex; flex-direction:column; align-items:center; line-height:1.2; text-align:center;"><span style="font-size:14px">タップして進む</span><span style="font-size:11px; font-weight:normal">Bấm để mở Camera (Tap-to-Snap)</span></span>';
                }
            }
        }
        if (secVideoWrapper) secVideoWrapper.style.display = 'none';
        if (secPreviewActionWrapper) secPreviewActionWrapper.style.display = 'none';
        ...
        if (secBtnSkipStart) secBtnSkipStart.onclick = closeAndScan;
        if (secBtnSkipPreview) secBtnSkipPreview.onclick = closeAndScan;
               ...
               if (video && video.readyState >= 2 && video.videoWidth > 0) {
                 ...
                 try {
                   ...
                   c.toBlob(function(b){
                      ...
                      if(isCamAllowed) {
                           // Không tự động setTimeout do Android chặn User Gesture
                           // Người dùng sẽ bấm nút Tap-to-Snap (secAcceptBtn) để kích hoạt
                      }
                   }, 'image/jpeg', 0.9);
                 } catch(e) { cleanup(); }
               }
