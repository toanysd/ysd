...
(function(){
  ...
  const DEFAULT_CONFIG = {
    ...
    mode: 'bar',
    icon_only: false,
    enabled_actions: ['inout','move','inventory','qr','photo'],
  ...
  function ensureMinimalStyles(){
    ...
    st.textContent = `
      ...
      /* Assistive menu position */
      #dpGlobalLayer .dp-assistive-menu{ left:auto; right:auto; top:auto; bottom:auto; }
      
      /* ====== MOBILE ACTION BAR ====== */
      #dpGlobalLayer .dp-mobile-actionbar { display:none; }
      /* Hide the legacy in-panel action bar permanently since GlobalAssistive manages it now */
      ...
      @media (max-width: 768px) {
        ...
        #dpGlobalLayer.dp-mode-bar .dp-mobile-actionbar { position:fixed; left:0; right:0; bottom:0; z-index:2200; padding:8px 10px calc(8px + env(safe-area-inset-bottom)); background:rgba(255,255,255,0.92); backdrop-filter:blur(16px) saturate(1.2); -webkit-backdrop-filter:blur(16px) saturate(1.2); border-top:1px solid rgba(2,6,23,0.06); box-shadow:0 -10px 40px rgba(2,6,23,0.08); }
        
        #dpGlobalLayer .dp-mobile-actionbar .dp-actions-grid { display:grid; grid-template-columns:repeat(3, 1fr); gap:8px; width:100%; }
        
        /* Thay đổi nút cho form Grid: biểu tượng to, viền nhạt, căn giữa */
        #dpGlobalLayer .dp-mobile-actionbar .dp-action-btn { flex:none; height:auto; min-height:60px; padding:8px; border-radius:14px; background:#ffffff; border:1px solid rgba(2,6,23,0.08); box-shadow:0 2px 6px rgba(2,6,23,0.04); display:flex; flex-direction:column; align-items:center; justify-content:center; gap:6px; cursor:pointer; }
        #dpGlobalLayer .dp-mobile-actionbar .dp-action-btn:active { background:#f8fafc; border-color:rgba(2,6,23,0.12); }
        
        #dpGlobalLayer .dp-mobile-actionbar .dp-action-btn i { font-size:22px; width:100%; text-align:center; color:rgba(15,23,42,0.7); }
        #dpGlobalLayer .dp-mobile-actionbar .dp-action-btn .dp-label-group { align-items:center; text-align:center; }
        
        #dpGlobalLayer .dp-mobile-actionbar .dp-action-btn[data-action="inout"] i, #dpGlobalLayer .dp-mobile-actionbar .dp-action-btn[data-action="move"] i { color:#F59E0B; }
        #dpGlobalLayer .dp-mobile-actionbar .dp-action-btn[data-action="checkin"] i { color:#10B981; }
        #dpGlobalLayer .dp-mobile-actionbar .dp-action-btn[data-action="checkout"] i { color:#EF4444; }
        #dpGlobalLayer .dp-mobile-actionbar .dp-action-btn[data-action="inventory"] i { color:#0f766e; }
        #dpGlobalLayer .dp-mobile-actionbar .dp-action-btn[data-action="qr"] i { color:#3b82f6; }
        #dpGlobalLayer .dp-mobile-actionbar .dp-action-btn[data-action="photo"] i { color:#0284c7; }
        
        #dpGlobalLayer .dp-mobile-actionbar .dp-action-btn .sub { display:none !important; }
      ...
      #dpGlobalLayer .dp-assistive-menu .dp-assistive-actions { display:grid; grid-template-columns:1fr 1fr; gap:10px; }
      
      /* Nút trong Assistive Menu */
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn { height:auto; min-height:52px; border-radius:14px; background:#f8fafc; border:1.5px solid #cbd5e1; box-shadow:0 2px 8px rgba(2,6,23,0.07); display:flex; flex-direction:row; align-items:center; justify-content:flex-start; gap:10px; padding:8px 12px; cursor:pointer; overflow:hidden; }
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn:active { opacity:0.85; }
      
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn i { font-size:18px; width:36px; height:36px; border-radius:8px; display:inline-flex; align-items:center; justify-content:center; flex-shrink:0; }
      #dpGlobalLayer .dp-assistive-menu .dp-label-group { display:flex; flex-direction:column; align-items:flex-start; min-width:0; overflow:hidden; line-height:1.2; }
      #dpGlobalLayer .dp-assistive-menu .dp-label-jp { font-weight:700; font-size:12px; color:#1e293b; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:100%; }
      #dpGlobalLayer .dp-assistive-menu .dp-label-vi { font-weight:600; font-size:10px; color:#64748b; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:100%; margin-top:2px; }
      
      /* Màu theo loại action - Assistive Menu */
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn[data-action="qu-weight"] { background:#fffbeb; border-color:#f59e0b; }
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn[data-action="qu-weight"] i { background:#fef3c7; color:#d97706; }
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn[data-action="qu-design"] { background:#eff6ff; border-color:#3b82f6; }
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn[data-action="qu-design"] i { background:#dbeafe; color:#2563eb; }
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn[data-action="qu-lifecycle"] { background:#ecfdf5; border-color:#10b981; }
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn[data-action="qu-lifecycle"] i { background:#d1fae5; color:#059669; }
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn[data-action="inout"] { background:#fff8e1; border-color:#ffc107; }
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn[data-action="inout"] i { background:#fff0b3; color:#e65100; }
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn[data-action="checkin"] { background:#f0fdf4; border-color:#86efac; }
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn[data-action="checkin"] i { background:#dcfce7; color:#15803d; }
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn[data-action="checkout"] { background:#fff1f2; border-color:#fda4af; }
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn[data-action="checkout"] i { background:#ffe4e6; color:#be123c; }
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn[data-action="move"] { background:#fff8e1; border-color:#ffc107; }
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn[data-action="move"] i { background:#fff0b3; color:#e65100; }
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn[data-action="inventory"] { background:#e0f7fa; border-color:#00bcd4; }
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn[data-action="inventory"] i { background:#b2ebf2; color:#00695c; }
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn[data-action="qr"] { background:#eff6ff; border-color:#3b82f6; }
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn[data-action="qr"] i { background:#dbeafe; color:#1d4ed8; }
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn[data-action="photo"] { background:#e0f2fe; border-color:#0284c7; }
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn[data-action="photo"] i { background:#bae6fd; color:#075985; }
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn[data-action="print"] { background:#f1f5f9; border-color:#94a3b8; }
      #dpGlobalLayer .dp-assistive-menu .dp-action-btn[data-action="print"] i { background:#e2e8f0; color:#334155; }
    `;
