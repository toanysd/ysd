...
document.addEventListener('click', (e) => {
  try {
    ...
    renderLabelHtml(labelRaw) {
      try {
        ...
        if (raw.includes('|||')) {
          ...
          if (jp && vi) {
            return `<span class="dp-label-jp">${this.escapeHtml(jp)}</span><span class="dp-label-vi">${this.escapeHtml(vi)}</span>`;
          }
    ...
    applyResizeState(grid, state){
      ...
      // Nếu đang ở tablet/mobile (CSS đang chuyển layout khác), bỏ mọi khóa width inline
      try{
        if (window.innerWidth < 769) {
          colLeft.style.width = '';
          colLeft.style.minWidth = '';
          colLeft.style.maxWidth = '';

          colRight.style.width = '';
          colRight.style.minWidth = '';
          colRight.style.maxWidth = '';

          colCenter.style.width = '';
          colCenter.style.minWidth = '';
      ...
      // CỘT GIỮA: để tự co giãn theo browser (không set width px)
      colCenter.style.width = '';
      colCenter.style.minWidth = minCenterW + 'px';

      // Template: trái px - giữa co giãn - phải px
      grid.style.gridTemplateColumns = `${left}px minmax(${minCenterW}px, 1fr) ${right}px`;
    }
    ...
    makeResizable(column, handle, side, grid) {
      ...
      const applyWidths = (leftW, centerW, rightW) => {
        const cols = getCols();
        const left = cols.left, center = cols.center, right = cols.right;
        ...
        // Center co giãn theo khung
        center.style.width = '';
        center.style.minWidth = minCenterW + 'px';

        grid.style.gridTemplateColumns = `${leftW}px minmax(${minCenterW}px, 1fr) ${rightW}px`;
      };
      ...
      handle.addEventListener('mousedown', (e) => {
        ...
        totalColumnsW = Math.round(grid.getBoundingClientRect().width) || (startLeftW + startCenterW + startRightW);
      ...
      html += `
        <div class="modal-section">
          ...
          <div class="info-grid-2col ">
            <div class="info-item">
              <div class="info-label">${this.renderLabelHtml(this.biLabel('状態', 'Status'))}</div>
              <div class="info-value">---</div>
            </div>
            <div class="info-item">
              <div class="info-label">${this.renderLabelHtml(this.biLabel('保管', 'Location'))}</div>
              <div class="info-value">---</div>
            </div>
            <div class="info-item">
              <div class="info-label">${this.renderLabelHtml(this.biLabel('移動', 'Transfer'))}</div>
              <div class="info-value">---</div>
            </div>
          </div>
          <div class="info-message" >
    ...
    renderDesktopQuickActions(item, type) {
      return `
        <div class="modal-section" style="margin-bottom:16px;">
          ...
          <div class="dp-actions-grid" style="grid-template-columns: 1fr 1fr 1fr;">
            <button class="dp-action-btn" data-action="qu-weight" type="button" title="重量更新 / Cập nhật Khối lượng">
              <i class="fas fa-weight-hanging" style="color:#f59e0b;"></i>
              <div style="display:flex; flex-direction:column; align-items:flex-start; line-height:1.2;">
                <span class="dp-action-label-ja" style="font-weight:700; font-size:13px; color:#1e293b;">重量</span>
                <span class="sub dp-action-label-vi" style="font-size:11px; font-weight:600; color:#64748b; margin-top:2px;">Khối lượng</span>
              </div>
            </button>
            <button class="dp-action-btn" data-action="qu-design" type="button" title="設計情報 / Thông tin Thiết kế">
              <i class="fas fa-ruler-combined" style="color:#3b82f6;"></i>
              <div style="display:flex; flex-direction:column; align-items:flex-start; line-height:1.2;">
                <span style="font-weight:700; font-size:13px; color:#1e293b;">設計</span>
                <span class="sub" style="font-size:11px; font-weight:600; color:#64748b; margin-top:2px;">Thiết kế</span>
              </div>
            </button>
            <button class="dp-action-btn" data-action="qu-lifecycle" type="button" title="運用状況 / Vận hành">
              <i class="fas fa-truck-loading" style="color:#10b981;"></i>
              <div style="display:flex; flex-direction:column; align-items:flex-start; line-height:1.2;">
                <span style="font-weight:700; font-size:13px; color:#1e293b;">運用</span>
                <span class="sub" style="font-size:11px; font-weight:600; color:#64748b; margin-top:2px;">Vận hành</span>
              </div>
            </button>
          </div>
        </div>
        ...
        <div class="modal-section">
          ...
          <div class="dp-actions-grid" style="grid-template-columns: repeat(3, 1fr);">
            <button class="dp-action-btn" data-action="inout" type="button" title="入出庫・位置変更 / Nhập xuất, chuyển vị trí">
              <i class="fas fa-map-marker-alt"></i>
              <div style="display:flex; flex-direction:column; align-items:flex-start; line-height:1.2;">
                <span class="dp-action-label-ja" style="font-weight:700; font-size:13px; color:#1e293b;">入出庫・位置</span>
                <span class="sub dp-action-label-vi" style="font-size:11px; font-weight:600; color:#64748b; margin-top:2px;">Nhập xuất, Vị trí</span>
              </div>
            </button>
            <button class="dp-action-btn" data-action="inventory" type="button" title="棚卸 / Kiểm kê">
              <i class="fas fa-clipboard-check"></i>
            ...
            <button class="dp-action-btn" data-action="teflon" type="button" title="テフロン / Mạ Teflon">
              <i class="fas fa-paint-roller" style="color:#0f766e;"></i>
              <div style="display:flex; flex-direction:column; align-items:flex-start; line-height:1.2;">
                <span class="dp-action-label-ja" style="font-weight:700; font-size:13px; color:#1e293b;">テフロン</span>
                <span class="sub dp-action-label-vi" style="font-size:11px; font-weight:600; color:#64748b; margin-top:2px;">Mạ Teflon</span>
              </div>
            </button>
            <button class="dp-action-btn" data-action="print" type="button" title="印刷 / In">
              <i class="fas fa-print"></i>
              <div style="display:flex; flex-direction:column; align-items:flex-start; line-height:1.2;">
                <span style="font-weight:700; font-size:13px; color:#1e293b;">印刷</span>
                <span class="sub" style="font-size:11px; font-weight:600; color:#64748b; margin-top:2px;">In</span>
              </div>
            </button>
            <button class="dp-action-btn" data-action="photo" type="button" title="写真 / Ảnh">
              <i class="fas fa-camera"></i>
              <div style="display:flex; flex-direction:column; align-items:flex-start; line-height:1.2;">
                <span style="font-weight:700; font-size:13px; color:#1e293b;">写真</span>
                <span class="sub" style="font-size:11px; font-weight:600; color:#64748b; margin-top:2px;">Ảnh</span>
            ...
            <button class="dp-action-btn" data-action="qr" type="button" title="QRコード / Mã QR">
              <i class="fas fa-qrcode"></i>
              <div style="display:flex; flex-direction:column; align-items:flex-start; line-height:1.2;">
                <span class="dp-action-label-ja" style="font-weight:700; font-size:13px; color:#1e293b;">QRコード</span>
                <span class="sub dp-action-label-vi" style="font-size:11px; font-weight:600; color:#64748b; margin-top:2px;">Mã QR</span>
              </div>
            </button>
          </div>
        </div>
      ...
      return `
        <div class="modal-section location-section">
          ...
          <div class="location-content">
            ...
            ` : `
            <div class="dp-storage-mobile-grid" style="display:grid;grid-template-columns:${this.isDesktopWide ? 'minmax(140px,44vw) minmax(0,1fr)' : '110px 1fr'};gap:12px;align-items:start">
              <div style="position:relative;border-radius:12px;overflow:hidden;border:1px solid rgba(15,23,42,0.10);background:#fff;min-height:140px;height:100%">
                <img data-dp-thumb-img="1"
                     src=""
                     alt=""
                     style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;display:block;opacity:0;transition:opacity .18s ease;cursor:pointer"
                     onerror="this.style.opacity='0'"/>
                <div data-dp-thumb-placeholder="1"
                     style="position:absolute;inset:0;padding:10px;color:#64748b;font-weight:800;display:flex;align-items:center;justify-content:center;cursor:pointer;text-align:center">
                  <div data-dp-photo-upload="1" style="display:flex;gap:8px;align-items:center">
                    <i class="fas fa-camera" style="opacity:.65"></i>
                    <div>
                      <div style="font-size:11px">写真なし。クリックして撮影/アップロード</div>
                      <div style="font-size:11px;opacity:.90;font-weight:900">Chưa có ảnh, bấm để tải/ chụp ảnh</div>
                    </div>
                  </div>
                </div>
              </div>

              <div class="info-grid-2col dp-kv-1col dp-kv-compact">
                <div class="info-item">
                  <div class="info-label">${this.renderLabelHtml(this.biLabel('保管会社','Công ty'))}</div>
                  <div class="info-value"><span class="${companyBadgeCls}">${e(companyInfo.nameShort || '-')}</span></div>
                </div>

                <div class="info-item">
                  <div class="info-label">${this.renderLabelHtml(this.biLabel('棚-段','Giá - Tầng'))}</div>
                  <div class="info-value"><span class="dp-badge-racklayer">${e(rackLayerText)}</span></div>
                </div>

                <div class="info-item">
                  <div class="info-label">${this.renderLabelHtml(this.biLabel('状態','Trạng thái'))}</div>
                  <div class="info-value">
                    <span class="status-badge-compact ${e(statusInfo.class || '')}">
                      <i class="${e(statusInfo.icon || '')}"></i>
                      <span>${e(statusInfo.textShort || '---')}</span>
                    </span>
                  </div>
                ...
                <div class="info-item">
                  <div class="info-label">${this.renderLabelHtml(this.biLabel('確認(棚)','Xác nhận vị trí'))}</div>
                  <div class="info-value">
                    <span class="date-badge-compact">
                      <i class="fas fa-calendar-check"></i>
                      <span class="date-text">${e(statusInfo.lastConfirmDateText || '---')}</span>
                    </span>
                  </div>
                ...
                <div class="info-item">
                  <div class="info-label">${this.renderLabelHtml(this.biLabel('位置','Vị trí'))}</div>
                  <div class="info-value">${rackLocationValueHtml}</div>
                </div>

                ${rackNotes ? `
                  <div class="info-item dp-storage-note">
                    <div class="info-label">${this.renderLabelHtml(this.biLabel('メモ(棚)','Ghi chú giá'))}</div>
                    <div class="info-value">${e(rackNotes)}</div>
                  </div>
                ` : ''}

                ${layerNotes ? `
                  <div class="info-item dp-storage-note">
                    <div class="info-label">${this.renderLabelHtml(this.biLabel('メモ(段)','Ghi chú tầng'))}</div>
                    <div class="info-value">${e(layerNotes)}</div>
                  </div>
                ` : ''}
              </div>
  ...
  (function () {
    ...
    function ensureStyle() {
      ...
      st.textContent = `
        ...
        .mcs-qv-body{flex:1 1 auto;position:relative;background:#0b1220;display:flex;align-items:center;justify-content:center;}
        .mcs-qv-img{max-width:100%;max-height:100%;object-fit:contain;display:none;}
