      ...
      const renderGrid = (arr, cols) => {
         ...
         for (const f of arr) {
           ...
           if (rawLbl.includes('|||')) {
              ...
              processedLabel = `${parts[0]} <span style="font-size:0.9em; color:#64748b; font-weight:normal; margin-left:4px;">(${parts[1]})</span>`;
           }
           
           html += `
             <div class="ov-item" style="display: flex !important; flex-direction: row !important; align-items: baseline !important; justify-content: space-between !important; padding: 10px 14px; background: rgba(248, 250, 252, 0.6); border: 1px solid rgba(15, 23, 42, 0.06); border-radius: 12px; gap: 8px;">
               <div class="info-label" style="border: none !important; padding: 0 !important; flex: 0 1 auto !important; margin-bottom: 0 !important; font-size: 11px !important; margin-right: 8px !important;">${processedLabel}</div>
               <div class="info-value" style="flex: 1 1 auto !important; text-align: right !important; word-break: break-word !important; padding: 0 !important; border: none !important; font-size: 14px !important;">${f.rawValue ?? '-'}</div>
             </div>
           `;
         }
         html += `</div>`;
...
print("Custom Inline Grid Replace Successful! 6")
