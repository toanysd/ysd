  ...
  PhotoManagerModule.prototype._mount = function () {
    ...
    if (!document._pmCtxV15) {
      ...
      document.addEventListener('click',function(e){
        ...
        if(inL){e.preventDefault();e.stopPropagation();pm._hideContextMenu();if(!pm._selMode)pm._openInfoPanel(rec);return;}
        if(pm._selMode) {
          if(it.tagName !== 'TR') { 
            e.preventDefault(); e.stopPropagation(); 
            var c = it.querySelector('.pm-item-check'); 
            if(c) c.click(); 
          }
          return;
        }
