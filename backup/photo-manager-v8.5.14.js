...
(function (global) {
  ...
  PhotoManagerModule.prototype._mount = function () {
    ...
    var pmC = document.getElementById('pmContent');
    if (pmC && !pmC.dataset.evts) {
      pmC.dataset.evts = '1';
      ...
      // Xử lý sự kiện chuột phải chuẩn mực (PC Mouse)
      pmC.addEventListener('contextmenu', function (e) {
        if (isLongPress) {
           e.preventDefault(); e.stopPropagation(); return;
        } // Đã xử lý bằng pointerdown timer

        var item = e.target.closest('.pm-item') || e.target.closest('tr');
        if (!item || !item.dataset.id) return;
        
        e.preventDefault();
        e.stopPropagation();
        var id = item.dataset.id;
        var rec = self._filtered.find(function(r) { return String(r.id) === String(id); });
        self._hideContextMenu();
        if (rec) self._showContextMenu(e, rec);
      }, true);

      // Bắt Long Press cho Mobile \u0026 Fix Window Focus cho PC
      pmC.addEventListener('pointerdown', function(e) {
         if (!document.hasFocus()) window.focus(); // Fix Windows OS focus swallow
         ...
         if (e.pointerType === 'touch') {
            ...
            longPressTimer = setTimeout(function() {
                ...
                if (rec) {
                    ...
                    self._showContextMenu({ clientX: touchStartPos.x, clientY: touchStartPos.y }, rec);
                }
            }, 550);
         }
      }, true);

      pmC.addEventListener('pointermove', function(e) {
         if (longPressTimer) {
             var dx = Math.abs(e.clientX - touchStartPos.x);
             var dy = Math.abs(e.clientY - touchStartPos.y);
             if (dx > 10 || dy > 10) {
      ...
      pmC.addEventListener('pointerup', function(e) {
         if (longPressTimer) { clearTimeout(longPressTimer); longPressTimer = null; }
      }, true);
      pmC.addEventListener('pointercancel', function(e) {
         if (longPressTimer) { clearTimeout(longPressTimer); longPressTimer = null; }
      }, true);

      pmC.addEventListener('click', function (e) {
        if (isLongPress) {
            e.preventDefault();
            e.stopPropagation();
            isLongPress = false;
        ...
        if (e.target.closest('.pm-item-check')) return;

        var inLabel = e.target.closest('.pm-item-label') || e.target.closest('.pm-device-link');
        ...
        if (item.tagName === 'TR') {
             ...
             var idx = self._filtered.indexOf(rec);
             if (idx >= 0) self._openLightbox(idx);
        }
      });
    }
