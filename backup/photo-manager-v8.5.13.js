...
(function (global) {
  ...
  PhotoManagerModule.prototype._mount = function () {
    ...
    if (pmC && !pmC.dataset.evts) {
      ...
      // Lột sạch lớp bảo vệ của trình duyệt, cấm nó phản ứng với chuột phải
      pmC.addEventListener('contextmenu', function (e) {
        e.preventDefault();
        e.stopPropagation();
      }, true);

      // Bắt chính xác từng nhịp chạm (kể cả chuột phải) bằng phần cứng thấp nhất
      pmC.addEventListener('pointerdown', function (e) {
        if (e.button === 2) { // 2 = Chuột phải
          var item = e.target.closest('.pm-item') || e.target.closest('tr');
          if (!item || !item.dataset.id) return;
          
          e.preventDefault();
          e.stopPropagation();
          var rec = self._filtered.find(function(r) { return String(r.id) === String(item.dataset.id); });
