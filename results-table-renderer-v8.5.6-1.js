...
class ResultsTableRenderer {
  constructor(containerId) {
    ...
    // Column resize
    this.colKeys = ['checkbox','no','code','name','dimensions','location','type','date','status','id','actions'];
    this.resizableCols = ['no','code','name','dimensions','location','id'];
      ...
      return `
        <tr data-id="${itemId}" data-type="${typeClass}" class="${isSelected ? 'selected' : ''}">
          ...
          <td class="col-actions">
            <button class="table-action-btn" data-id="${itemId}" data-type="${typeClass}" aria-label="actions">
      ...
      row.addEventListener('touchstart', (e) => {
        pressTimer = window.setTimeout(() => {
          ...
          if (item) {
             ...
             // Giả lập MouseEvent object truyền cho openTableActionMenu
...
@media (max-width: 768px){
  ...
  .results-table th.col-date, .results-table td.col-date,
