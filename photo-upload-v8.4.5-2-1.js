/* ============================================================
   PHOTO UPLOAD v8.4.5-2-1
   Module chụp / upload / chỉnh sửa ảnh khuôn–dao cắt
...
(function (global) {
  ...
  var TMPL_MAIN = [
    ...
    '      <button class="pu-photo-action-btn pu-btn-camera" id="puCameraBtn"><i class="fas fa-camera"></i> 撮影 / Chụp</button>',
    '      <button class="pu-photo-action-btn" id="puFileBtn"><i class="fas fa-folder-open"></i> ファイル / Chọn file</button>',
    '      <input type="file" id="puFileInput" accept="image/*" multiple style="display:none">',
    '    </div>',
    /* Drop zone */
    ...
    '        <button class="pu-confirm-btn pu-confirm-primary" id="puMissingCodeQuickBtn" type="button">unknown_ で送信 / Gửi nhanh unknown_</button>',
    ...
    /* Footer */
    '<div class="pu-footer">',
    '  <button class="pu-send-btn" id="puSendBtn">',
    '    <i class="fas fa-paper-plane pu-send-icon"></i>',
    '    <div class="pu-spinner"></div>',
    '    <span id="puSendBtnText">送信 / Gửi &amp; Lưu</span>',
    '  </button>',
    '  <button class="pu-save-only-btn" id="puSaveOnlyBtn"><i class="fas fa-save"></i> 保存のみ / Chỉ lưu</button>',
  ...
  PhotoUploadModule.prototype._reset = function () {
    ...
    var sendBtn = document.getElementById('puSendBtn');
    if (sendBtn) { sendBtn.disabled = false; sendBtn.classList.remove('pu-sending'); }
  ...
  PhotoUploadModule.prototype._applyDeviceContext = function () {
    ...
    /* Badge */
    var badge = document.getElementById('puDeviceBadge');
    if (badge) { badge.textContent = (d.type === 'mold' ? '🔧 ' : '✂️ ') + d.code; badge.classList.remove('pu-hidden'); }
  ...
  PhotoUploadModule.prototype._bindStatic = function () {
    ...
    /* Send & Save buttons */
    var sendBtn = document.getElementById('puSendBtn');
    if (sendBtn) sendBtn.addEventListener('click', function () { self._submit(true); });
    var saveOnlyBtn = document.getElementById('puSaveOnlyBtn');
    if (saveOnlyBtn) saveOnlyBtn.addEventListener('click', function () { self._submit(false); });
  ...
  PhotoUploadModule.prototype._editorApply = function () {
    ...
    self._processImage(this._editor.baseBlob, {
      angle: this._editor.angle,
      flipH: this._editor.flipH,
      flipV: this._editor.flipV
    }, this._resizeMode).then(function (blob) {
      var url = URL.createObjectURL(blob);
    ...
    }).catch(function () { self._closeEditor(); });
  };
  ...
  /* ──────────────────────────────────────────────────────────
     SUBMIT
  ────────────────────────────────────────────────────────── */
  PhotoUploadModule.prototype._submit = function (sendMail) {
    ...
    if (!_codeNow.trim()) {
      this._showMissingCodeDialog(function () {
        ...
        setTimeout(function () { self._submit(sendMail); }, 0);
      });
      return;
    }
    ...
    var deviceType  = (this._device && this._device.type) || 'mold';
    ...
    var sendBtn = document.getElementById('puSendBtn');
    if (sendBtn) { sendBtn.disabled = true; sendBtn.classList.add('pu-sending'); }
    var pw = document.getElementById('puProgressWrap');
    ...
    function next(i) {
      if (i >= total) {
        self._onAllUploaded(results, errors, {
          code: code, dims: dims, notes: notes,
          senderId: senderId, senderName: senderName,
          sendMail: sendMail, isManualCode: isManualCode, isManualDim: isManualDim
        });
        return;
      }
  ...
  PhotoUploadModule.prototype._onAllUploaded = function (results, errors, meta) {
    ...
    if (!client || typeof client.functions !== 'object') {
      ...
      this._finalize(results, errors); return;
    }
    ...
    // ----- build photos for Edge Function (quan trọng: fileName = storagepath) -----
  ...
  PhotoUploadModule.prototype._finalize = function (results, errors) {
    ...
    var msg = results.length + ' 枚アップロード完了 / ' + results.length + ' ảnh đã lưu';
